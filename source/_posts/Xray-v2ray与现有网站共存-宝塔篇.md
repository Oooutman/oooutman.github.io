...
#### 教程分为 4 步，复制粘贴几下就搞定了。
{% note info %}
1. 官方脚本手动安装 Xray/v2ray（教程以 Xray 为例，V2ray 除了这个安装脚本不同外其它基本一样）
2. 编辑 config.json
...
### 3. Nginx 设置反向代理
宝塔进入网站设置的配置文件，拉到最后面在最后一个 `}` 前面插入以下部分，需要把下面的 `/a1fih5` 和 `10000` 修改成和上面配置一样的路径和端口，宝塔的这个站点配置改完保存即可生效，手搓可能要重载配置才能生效。
